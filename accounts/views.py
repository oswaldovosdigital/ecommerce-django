    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            body = render_to_string('accounts/account_verification_email.html',{
                'user': user,
                'domain': current_site,
                'uid': urlsafe_base64_encode(force_bytes(user.pk)),#transformarlo a asdas343234ad
...
def forgotPassword(request):
    ...
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            message.error(reuqest, 'La cuenta de usuario no existe')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            messages.error(reuqest, 'El password de confirmacion no concuerda')
            return redirect('resetPassword')
    else:
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.error(request, 'Por favor ingrese un password validos')
                return redirect('change_password')
        else:
